...
void mod21ex2() {
	...
	int countLands, countBuilding, choise, countRooms;
	int buf;
	...
	for (int i = 0; i < countLands; i++) {
		...
		for (int j = 0; j < countBuilding; j++) {
			...
			std::cin >> choise;
			lands[i].buildingInLand[j].type = static_cast<typeBuilding>(choise - 1);
				...
				for (int k = 0; k < lands[i].buildingInLand[j].countLevel; k++) {
					do {
						...
						if (choise == 1) {
							...
							std::cin >> choise;
							lands[i].buildingInLand[j].rooms[countRooms++].type = static_cast<typeRoom>(choise - 1);
						}
						else {
							break;
						}
